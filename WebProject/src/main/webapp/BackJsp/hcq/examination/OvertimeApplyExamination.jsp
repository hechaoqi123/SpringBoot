...
<td style="TEXT-ALIGN: center"><span style="COLOR: rgb(255,0,0)">*</span>共计时长</td>
<td id="dbf.text5" dbf.type="required" dbf.source="">
 ${ apply.field1}
</td></tr>
<tr>
<td style="TEXT-ALIGN: center"><span style="COLOR: rgb(255,0,0)">*</span>加班事由</td>
<td id="加班事由" style="HEIGHT: 80px" colspan="5" dbf.type="required" dbf.source="">
   ${apply.cause}
</td></tr>
...
<div style="padding:2px 10px;"><div style="float:right;"><a href="javaScript:" onclick="javaScript:windowOpen(&#39;../flow/view.htm?1000019,1000481&#39;);return false;">› 显示流程图</a></div>【处理过程】</div>
  <span v-for="app in approval"> 
    ...
    <div v-show="app.sequence==1" style="padding:0px 10px 5px 30px;">[<b>领导审批</b>] {{app.remark}}</div>
    <div v-show="app.sequence==2" style="padding:0px 10px 5px 30px;">[<b>人事处理</b>] {{app.remark}}</div>
    <div v-show="app.sequence==3" style="padding:0px 10px 5px 30px;">[<b>结束流程</b>] {{app.remark}}</div>
    <div v-show="app.sequence==4" style="padding:0px 10px 5px 30px;">[<b>驳回</b>] {{app.remark}}</div>
  </span>
</td><td></td><td>
...
<input id="recruitid" style="display:none" value="${apply.overtimeid}"/>
  ...
  new Vue({
              ...
              data:{
                 ...
                 getAll:function(){//流程
                     ...
                     this.$http.post(url,{itemname:"加班申请",itemid:itemid},{emulateJSON:true}).then(function(res){
                     ...
                     var url="Overtime/update";
                     if(obj=="领导"){//转领导审核
                       this.$http.post(url,{applypeople:principal,overtimeid:itemid,status:"领导审批",remark:remark},{emulateJSON:true}).then(function(res){
                           window.location.href="BackJsp/hcq/examination/check.jsp";                   
                       })  
                     }else if(obj=="人事"){//转人事
                       this.$http.post(url,{applypeople:principal,overtimeid:itemid,status:"人事处理",remark:remark},{emulateJSON:true}).then(function(res){
                           window.location.href="BackJsp/hcq/examination/check.jsp";                
                       })  
                     }else if(obj=="结束"){//结束流程
                       this.$http.post(url,{applypeople:principal,overtimeid:itemid,status:"结束",remark:remark},{emulateJSON:true}).then(function(res){
                           window.location.href="BackJsp/hcq/examination/check.jsp";                
                       })  
                     }else{//驳回
                       this.$http.post(url,{applypeople:principal,overtimeid:itemid,status:"驳回",remark:remark},{emulateJSON:true}).then(function(res){
                           window.location.href="BackJsp/hcq/examination/check.jsp";                
                       }) 
                     }
                 }
