...
<html>
  ...
  <span id="oWorkflowList1"><button id="save" class="btn" style="padding:5px 20px;border:1px solid #E0E0E0;background:#FCFCFC;border-radius:3px;cursor: pointer "><b>+</b>入职申请</button></span></td></tr></tbody></table><br>
    <!-- 分类 -->
	<div id="colloaMenu2">
	   <a  href="BackJsp/hcq/recruit.jsp">
	     <img border="0" src="BackJsp/hcq/img/folder.png"> 招聘管理</a>
	   <a id="ruz" class="textHighlight" href="BackJsp/hcq/entryPosition.jsp">
...
<span id="app">
<table style="margin-left:-15px;" cellpadding="0" cellspacing="0" border="0" class="tableList"><thead><tr><th>主题</th><th>入职人</th><th>入职日期</th><th>所属部门</th><th>入职岗位</th></tr></thead>
<tbody>
<tr v-for="entry in entrys">
<td>
<img v-if="entry.status == '结束'" width="16" src="BackJsp/hcq/img/ico2.png"/>
<img v-else width="16" src="BackJsp/hcq/img/ico1.png"/>
<a href="javaScript:showItem(&39;事务&39;,&39;1000165&39;);">{{entry.theme}}</a></td><td>{{entry.entryname}}</td><td>{{entry.entrydate}}</td><td>{{entry.entrypart}}&nbsp;</td><td>{{entry.entrypost}}</td></tr>
<tr>
</table><br/>
<!-- 分页 -->
...
<script>
     $(function(){
     $("#save").click(function(){
        window.location.href="BackJsp/hcq/entryApply.jsp"
     })
       var load=new Vue({
            el:'#app',
            data:{
