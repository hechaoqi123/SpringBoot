...
<tr>
<td style="text-align:center" colspan="1">备注</td>
<td   colspan="10" dbf.type="" dbf.source="">
 ...
 <c:if test="${apply.status!='结束'}">
  ...
  <div style="padding:2px 10px;"><div style="float:right;"><a href="javaScript:" ></a></div>【处理过程】</div>
   ...
   <div id="approval">
    ...
    <div v-show="app.sequence==0" style="padding:0px 10px 5px 30px;">[<b>提交领导审批</b>]{{app.remark}} </div>
    <div v-show="app.sequence==2" style="padding:0px 10px 5px 30px;">[<b>通过并转财务处理</b>] {{app.remark}}</div>
           ...
           new Vue({
              ...
              methods:{
                 submit:function(obj){
                     ...
                     if(obj=="财务"){//转财务
                       this.$http.post(url,{createpeople:principal,applyid:id,status:"财务处理",remark:remark},{emulateJSON:true}).then(function(res){
                           window.location.href="BackJsp/hcq2/examination/costManager.jsp";                
                       })  
                     }else if(obj=="结束"){//结束
                       this.$http.post(url,{createpeople:principal,applyid:id,status:"结束",remark:remark},{emulateJSON:true}).then(function(res){
                           window.location.href="BackJsp/hcq2/examination/costManager.jsp";                
                       })  
                     }else{//驳回
                       this.$http.post(url,{createpeople:principal,applyid:id,status:"驳回",remark:remark},{emulateJSON:true}).then(function(res){
                           window.location.href="BackJsp/hcq2/examination/costManager.jsp";                
                       }) 
                     }
                 }
